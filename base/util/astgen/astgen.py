  ...
  def __init__(self, parent, name, yaml_map):
    ...
    for (field, user_type) in self.elements.iteritems():
      if not isinstance(field, basestring):
        raise Exception("Field name must be a string in node type " + name + ", not: " + str(field))
      if not isinstance(user_type, basestring):
        raise Exception("Field type must be a string in node type " + name + ", not: " + str(user_type))
...
class Group(object):
  ...
  def __init__(self, parent, name, yaml_map):
    ...
    for (subtype, submap) in yaml_map.iteritems():
      if not isinstance(subtype, basestring):
        raise Exception("Subtype name must be a string in group type " + subtype+ ", not: " + str(subtype))
      if not isinstance(submap, dict):
        raise Exception("All types must be a yaml map, invalid type " + subtype + ", not: " + str(submap))
      if len(submap) == 0:
        raise Exception("Empty types not allowed: " + subtype)
      if isinstance(submap.itervalues().next(), basestring):
  ...
  def get_group_names(self):
    return set.union(set([self.name]), *[subtype.get_group_names() for subtype in self.subtypes.values()])

  def get_nodes(self, node_names, group_names):
    return sum([subtype.get_nodes(node_names, group_names) for subtype in self.subtypes.values()], [])
  ...
  def get_primitives(self, node_names, group_names):
    return set.union(*[subtype.get_primitives(node_names, group_names) for subtype in self.subtypes.values()])
