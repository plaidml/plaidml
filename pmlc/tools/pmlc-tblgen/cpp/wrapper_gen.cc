...
// The OpEmitter class is responsible for emitting the fluent EDSL code for each
// TableGen Record. It begins by querying the Record for relevant information
// about the Operator/Attributes/Results/Operands, then formats the information
// in in an EDSL-readable format.
class OpEmitter {
private:
  OpInfo opInfo_;
...
void OpEmitter::emitConstructor(raw_ostream &os) {
  // Declare the types of parameters that must be passed into the constructor.
  // Get this from the operands.
  ...
  os << ") : ";
  // Initialize the private variables in the op class with the parameters passed
  // in
  visited = false;
  for (const auto &operandPair : opInfo_.operands_) {
    if (visited)
...
void OpEmitter::emitDeclarations(raw_ostream &os) {
  for (const auto &operandPair : opInfo_.operands_) {
    os.indent(4) << convertType(operandPair.second) << " " << operandPair.first
                 << "_;\n";
  }
  for (const auto &attributePair : opInfo_.attributes_) {
    os.indent(4) << convertType(attributePair.second) << " "
                 << attributePair.first << "_;\n";
  }
}

void OpEmitter::emitOperatorOverload(raw_ostream &os) {
  os.indent(4) << "operator " << convertType(opInfo_.returnType_)
               << "() const {\n";
...
void OpEmitter::emitSetters(raw_ostream &os) {
  ...
  for (const auto &attributePair : opInfo_.attributes_) {
    os.indent(4) << opInfo_.name_ << " " << attributePair.first << "("
                 << convertType(attributePair.second) << " "
                 << attributePair.first << ") {\n";
...
TypeEmitter::TypeEmitter(const TypeInfo &type, raw_ostream &os)
    : typeInfo_(TypeInfo(type)) {
  os << formatv(commentHeader, typeInfo_.name_, "enumerator");
  os << "enum class " << typeInfo_.name_ << " : "
     << convertType(typeInfo_.returnType_) << " { \n";
