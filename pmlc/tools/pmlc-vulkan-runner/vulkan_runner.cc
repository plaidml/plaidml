...
class SPIRVModuleInfoCollector {
 public:
  ...
  ResourceStorageClassData& getResourceStorageClassData() { return resourceStorageClassData; }
...
void SPIRVModuleInfoCollector::processEntryPoint(spirv::EntryPointOp op) { entryPoint = op.fn(); }

/// Processes GlobalVariableOp to collect storage classes for resource data.
void SPIRVModuleInfoCollector::processGlobalVariable(spirv::GlobalVariableOp varOp) {
  ...
  if (descriptorSet && binding) {
    if (auto ptrType = varOp.type().dyn_cast<spirv::PointerType>()) {
      ...
      resourceStorageClassData[descriptorSetIndex][descriptorBindingIndex] = ptrType.getStorageClass();
    }
  }
}
}  // namespace
...
class VulkanRuntime {
 ...
 private:
  ...
  LogicalResult mapStorageClassToDescriptorType(spirv::StorageClass storageClass, VkDescriptorType& descriptorType);

  /// Maps storage class to buffer usage flags.
  LogicalResult mapStorageClassToBufferUsageFlag(spirv::StorageClass storageClass, VkBufferUsageFlagBits& bufferUsage);
  ...
  llvm::DenseMap<DescriptorSetIndex, llvm::SmallVector<VulkanDeviceMemoryBuffer, 1>> deviceMemoryBufferMap;

  /// Specifies shader module.
  VkShaderModule shaderModule;

  /// Specifies layout bindings.
  llvm::DenseMap<DescriptorSetIndex, llvm::SmallVector<VkDescriptorSetLayoutBinding, 1>> descriptorSetLayoutBindingMap;
...
void VulkanRuntime::setNumWorkGroups(const NumWorkGroups& nWorkGroups) { numWorkGroups = nWorkGroups; }

void VulkanRuntime::setResourceStorageClassData(const ResourceStorageClassData& stClassData) {
  resourceStorageClassData = stClassData;
}

void VulkanRuntime::setEntryPoint(llvm::StringRef entryPointName) { entryPoint = entryPointName; }

void VulkanRuntime::setResourceData(const ResourceData& resData) { resourceData = resData; }
...
LogicalResult VulkanRuntime::mapStorageClassToDescriptorType(spirv::StorageClass storageClass,
...
LogicalResult VulkanRuntime::mapStorageClassToBufferUsageFlag(spirv::StorageClass storageClass,
...
LogicalResult VulkanRuntime::run() {
  ...
  if (failed(createInstance()) || failed(createDevice()) || failed(createMemoryBuffers()) ||
...
LogicalResult VulkanRuntime::createDevice() {
  ...
  RETURN_ON_VULKAN_ERROR(vkEnumeratePhysicalDevices(instance, &physicalDeviceCount, 0), "vkEnumeratePhysicalDevices");

  llvm::SmallVector<VkPhysicalDevice, 1> physicalDevices(physicalDeviceCount);
  RETURN_ON_VULKAN_ERROR(vkEnumeratePhysicalDevices(instance, &physicalDeviceCount, physicalDevices.data()),
                         "vkEnumeratePhysicalDevices");

  RETURN_ON_VULKAN_ERROR(physicalDeviceCount ? VK_SUCCESS : VK_INCOMPLETE, "physicalDeviceCount");
  ...
  RETURN_ON_VULKAN_ERROR(vkCreateDevice(physicalDevice, &deviceCreateInfo, 0, &device), "vkCreateDevice");
  ...
  for (uint32_t i = 0, e = properties.memoryTypeCount; i < e; ++i) {
    if ((VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT & properties.memoryTypes[i].propertyFlags) &&
        ...
        (memorySize <= properties.memoryHeaps[properties.memoryTypes[i].heapIndex].size)) {
      memoryTypeIndex = i;
      break;
    }
  }

  RETURN_ON_VULKAN_ERROR(memoryTypeIndex == VK_MAX_MEMORY_TYPES ? VK_INCOMPLETE : VK_SUCCESS,
                         "invalid memoryTypeIndex");
  return success();
}

LogicalResult VulkanRuntime::getBestComputeQueue(const VkPhysicalDevice& physicalDevice) {
  uint32_t queueFamilyPropertiesCount = 0;
  vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice, &queueFamilyPropertiesCount, 0);
  SmallVector<VkQueueFamilyProperties, 1> queueFamilyProperties(queueFamilyPropertiesCount);
...
LogicalResult VulkanRuntime::createMemoryBuffers() {
  ...
  for (const auto& resourceDataMapPair : resourceData) {
    ...
    for (const auto& resourceDataBindingPair : resourceDataMap) {
      ...
      const auto resourceStorageClassMapIt = resourceStorageClassData.find(descriptorSetIndex);
      if (resourceStorageClassMapIt == resourceStorageClassData.end()) {
        llvm::errs() << "cannot find storge class for resource in descriptor set: " << descriptorSetIndex;
        return failure();
      }
      ...
      const auto resourceStorageClassIt = resourceStorageClassMap.find(resourceDataBindingPair.first);
      if (resourceStorageClassIt == resourceStorageClassMap.end()) {
        llvm::errs() << "cannot find storage class for resource with descriptor index: "
                     << resourceDataBindingPair.first;
        return failure();
      }

      const auto resourceStorageClassBinding = resourceStorageClassIt->second;
      if (failed(mapStorageClassToDescriptorType(resourceStorageClassBinding, descriptorType)) ||
          failed(mapStorageClassToBufferUsageFlag(resourceStorageClassBinding, bufferUsage))) {
        llvm::errs() << "storage class for resource with descriptor binding: " << resourceDataBindingPair.first
                     << " in the descriptor set: " << descriptorSetIndex << " is not supported ";
        return failure();
      }
      ...
      RETURN_ON_VULKAN_ERROR(
          vkMapMemory(device, memoryBuffer.deviceMemory, 0, bufferSize, 0, reinterpret_cast<void**>(&payload)),
          "vkMapMemory");
      ...
      RETURN_ON_VULKAN_ERROR(vkCreateBuffer(device, &bufferCreateInfo, 0, &memoryBuffer.buffer), "vkCreateBuffer");

      // Bind buffer and device memory.
      RETURN_ON_VULKAN_ERROR(vkBindBufferMemory(device, memoryBuffer.buffer, memoryBuffer.deviceMemory, 0),
                             "vkBindBufferMemory");
...
LogicalResult VulkanRuntime::createShaderModule() {
  ...
  RETURN_ON_VULKAN_ERROR(vkCreateShaderModule(device, &shaderModuleCreateInfo, 0, &shaderModule),
...
void VulkanRuntime::initDescriptorSetLayoutBindingMap() {
  for (const auto& deviceMemoryBufferMapPair : deviceMemoryBufferMap) {
    ...
    descriptorSetLayoutBindingMap[descriptorSetIndex] = descriptorSetLayoutBindings;
  }
}
...
LogicalResult VulkanRuntime::createDescriptorSetLayout() {
  for (const auto& deviceMemoryBufferMapPair : deviceMemoryBufferMap) {
    ...
    VkDescriptorType descriptorType = deviceMemoryBuffers.front().descriptorType;
    const uint32_t descriptorSize = deviceMemoryBuffers.size();
    const auto descriptorSetLayoutBindingIt = descriptorSetLayoutBindingMap.find(descriptorSetIndex);

    if (descriptorSetLayoutBindingIt == descriptorSetLayoutBindingMap.end()) {
      llvm::errs() << "cannot find layout bindings for the set with number: " << descriptorSetIndex;
      return failure();
    }

    const auto& descriptorSetLayoutBindings = descriptorSetLayoutBindingIt->second;
    ...
    RETURN_ON_VULKAN_ERROR(vkCreateDescriptorSetLayout(device, &descriptorSetLayoutCreateInfo, 0, &descriptorSetLayout),
                           "vkCreateDescriptorSetLayout");

    descriptorSetLayouts.push_back(descriptorSetLayout);
    descriptorSetInfoPool.push_back({descriptorSetIndex, descriptorSize, descriptorType});
  }
  return success();
}
...
LogicalResult VulkanRuntime::createPipelineLayout() {
  ...
  RETURN_ON_VULKAN_ERROR(vkCreatePipelineLayout(device, &pipelineLayoutCreateInfo, 0, &pipelineLayout),
...
LogicalResult VulkanRuntime::createDescriptorPool() {
  ...
  RETURN_ON_VULKAN_ERROR(vkCreateDescriptorPool(device, &descriptorPoolCreateInfo, 0, &descriptorPool),
...
LogicalResult VulkanRuntime::allocateDescriptorSets() {
  ...
  RETURN_ON_VULKAN_ERROR(vkAllocateDescriptorSets(device, &descriptorSetAllocateInfo, descriptorSets.data()),
...
LogicalResult VulkanRuntime::setWriteDescriptors() {
  ...
  for (const auto& descriptorSetInfo : descriptorSetInfoPool) {
    ...
    const auto& deviceMemoryBuffers = deviceMemoryBufferMap[descriptorSetInfo.descriptorSet];
...
LogicalResult VulkanRuntime::createCommandPool() {
  ...
  RETURN_ON_VULKAN_ERROR(vkCreateCommandPool(device, &commandPoolCreateInfo, 0, &commandPool), "vkCreateCommandPool");
  return success();
}
...
LogicalResult VulkanRuntime::createComputeCommandBuffer() {
  ...
  RETURN_ON_VULKAN_ERROR(vkAllocateCommandBuffers(device, &commandBufferAllocateInfo, &commandBuffer),
  ...
  // Commands begin.
  RETURN_ON_VULKAN_ERROR(vkBeginCommandBuffer(commandBuffer, &commandBufferBeginInfo), "vkBeginCommandBuffer");

  // Commands.
  vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline);
  vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipelineLayout, 0, descriptorSets.size(),
                          descriptorSets.data(), 0, 0);
  vkCmdDispatch(commandBuffer, numWorkGroups.x, numWorkGroups.y, numWorkGroups.z);

  // Commands end.
  RETURN_ON_VULKAN_ERROR(vkEndCommandBuffer(commandBuffer), "vkEndCommandBuffer");
...
LogicalResult VulkanRuntime::submitCommandBuffersToQueue() {
  ...
  RETURN_ON_VULKAN_ERROR(vkQueueSubmit(queue, 1, &submitInfo, 0), "vkQueueSubmit");
  return success();
}
...
LogicalResult VulkanRuntime::updateHostMemoryBuffers() {
  ...
  for (auto& resourceDataMapPair : resourceData) {
    ...
    auto& deviceMemoryBuffers = deviceMemoryBufferMap[resourceDataMapPair.first];
    ...
    for (auto& deviceMemoryBuffer : deviceMemoryBuffers) {
      if (resourceDataMap.count(deviceMemoryBuffer.bindingIndex)) {
        void* payload;
        auto& hostMemoryBuffer = resourceDataMap[deviceMemoryBuffer.bindingIndex];
        RETURN_ON_VULKAN_ERROR(vkMapMemory(device, deviceMemoryBuffer.deviceMemory, 0, hostMemoryBuffer.size, 0,
...
static LogicalResult runOnVulkan(spirv::ModuleOp module, ResourceData& resourceData,
  ...
  runtime.setResourceStorageClassData(moduleHandler.getResourceStorageClassData());

  if (failed(runtime.initRuntime()) || failed(runtime.run()) || failed(runtime.updateHostMemoryBuffers()) ||
