...
Rational UnifiedOffset(const Rational &c1, const Rational &c2,
                       const Integer &n1, const Integer &n2) {
  std::set<Rational> offsets;
  if (n1 > std::numeric_limits<std::size_t>::max() ||
      n2 > std::numeric_limits<std::size_t>::max()) {
    throw std::out_of_range(
        "Cannot unify offset when relative quotient exceeds size_t.");
  }
  ...
  IVLOG(1, "Failed to compute UnifiedOffset(" << c1 << ", " << c2 << ", " << n1
                                              << ", " << n2 << ").");
  throw std::runtime_error("Merging constraints with empty intersection.");
}

RangeConstraint
IntersectParallelConstraintPairInner(const RangeConstraint &constraint1,
  ...
  if (ratio == 0) {
    throw std::invalid_argument(
        "Parameters of IntersectParallelConstraintPair must be parallel");
  }
  ...
  if (constraint2.range > 0) {
    ...
    // Same calculations of lower/upper_bound as above, but with
    // constraint2.range == infinity
  ...
  for (const auto &kvp : map_) {
    if (kvp.first == "") {
      ...
      throw std::runtime_error(
          llvm::formatv("Failed to find value for {0}, when evaluating {1}",
                        kvp.first, toString()));
    }
  }
  return res;
...
template <typename T>
Polynomial<T>
Polynomial<T>::partial_eval(const std::map<std::string, T> &values) const {
  Polynomial<T> r = *this;
  T off = 0;
...
template <typename T>
void Polynomial<T>::substitute(const std::string &var,
...
template <typename T>
void Polynomial<T>::substitute(
    const std::map<std::string, Polynomial<T>> &replacements) {
  Polynomial result;
...
template <typename T>
Polynomial<T>
Polynomial<T>::sym_eval(const std::map<std::string, Polynomial> values) const {
  Polynomial<T> out;
  for (const auto &kvp : map_) {
    if (kvp.first.empty()) {
...
std::string Polynomial<T>::GetNonzeroIndex() const {
  // Returns a nonconstant nonzero index, if one exists; otherwise returns empty
  // string
  for (const auto &kvp : map_) {
    if (!(kvp.first.empty()) && kvp.second != 0)
      return kvp.first;
  }
...
SimpleConstraint::SimpleConstraint(const Polynomial<Rational> &poly,
                                   int64_t rhs)
    : poly(poly), rhs(rhs) {}

RangeConstraint::RangeConstraint(const Polynomial<Rational> &poly,
                                 int64_t range)
    : poly(poly), range(range) {}

bool RangeConstraint::IsParallel(const RangeConstraint &c) {
  if (this->poly.tryDivide(c.poly, true) != 0)
    return true;
  return false;
}

SimpleConstraint RangeConstraint::lowerBound() const {
  return SimpleConstraint(-poly, 0);
}

SimpleConstraint RangeConstraint::upperBound() const {
  return SimpleConstraint(poly, range - 1);
}
...
IntersectParallelConstraintPair(const RangeConstraint &constraint1,
  ...
  return IntersectParallelConstraintPairInner(
      constraint1, RangeConstraint(constraint2.rhs - constraint2.poly, -1));
}
...
IntersectOpposedSimpleConstraints(const SimpleConstraint &constraint1,
                                  const SimpleConstraint &constraint2) {
  // Combines two SimpleConstraints which are parallel and bounding in opposite
  // directions into a single equivalent RangeConstraint
  IVLOG(6, "Merging Opposed SimpleConstraints: " << constraint1 << " and "
                                                 << constraint2);
  Rational ratio = constraint1.poly.tryDivide(constraint2.poly, true);
  if (ratio >= 0) {
    throw std::invalid_argument(
  ...
  // (noting that this is just for the bounds, not the integrality, and noting
  // that the right hand bound will be slightly too generous if the RHS is not
  // an integer; see discussion below for why this is ok) And so use this to
  // make a range constraint from constraint1 and then call to
  // IntersectParallelConstraintPair
  ...
  // Note that if the range is too generous the IntersectParallel call will fix
  // it. So, instead of figuring out whether there is a fractional offset in the
  // new form and determining the precise range, use Ceil and Floor to get an
  // overapproximation of the range. This approach may duplicate a bit of work,
  // but I think this is too tiny for that to matter for overall perf
  auto range =
      math::Ceil(merged_const1) - math::Floor(ratio * merged_const2) + 1;
  if (range > INT64_MAX) {
    throw std::out_of_range(
        "Bound range in IntersectOpposedSimpleConstraints overflows int64.");
  }
  int64_t r = (int64_t)range;
  // IVLOG(6, "Ratio: " << ratio << "; merged_poly1: " << merged_poly1 << ";
  // merged_const1: " << merged_const1
  //                    << "; merged_const2: " << merged_const2);
  // IVLOG(6, "Range (& formula): " << range << " = ceil(" << merged_const1 <<
  // ") - floor(" << ratio << " * "
  //                                << merged_const2 << ")");
