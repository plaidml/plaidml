...
public:
  ...
  Tableau(const math::Matrix &m, const std::vector<std::string> &var_names,
          const std::vector<size_t> *opposites = nullptr);
  // Construct an empty Tableau of dimension size1 x size2
  Tableau(math::Matrix::size_type size1, math::Matrix::size_type size2,
          const std::vector<std::string> &var_names,
          const std::vector<size_t> *opposites = nullptr);
  // Accessors
  // Don't use resize or swap on mat(); the other parts of Tableau won't
  // recognize this and will fail
  const math::Matrix &mat() const { return matrix_; }
  math::Matrix &mat() { return matrix_; }
  ...
  // (i.e. contains permuted identity submatrix, last column nonnegative except
  // possibly objective)
  ...
  // Solution reporting functions. These report based on the current state of
  // the tableau, so if it hasn't been solved they are likely meaningless
  ...
  math::Rational reportObjectiveValue() const;
