...
Tableau::Tableau(const Matrix &m, const std::vector<std::string> &var_names,
...
Tableau::Tableau(Matrix::size_type size1, Matrix::size_type size2,
                 const std::vector<std::string> &var_names,
                 const std::vector<size_t> *opposites)
    : matrix_(size1, size2), var_names_(var_names),
      opposites_(var_names.size(), 0) {
  if (opposites) {
    opposites_ = *opposites;
  } else {
    buildOppositesFromNames();
  ...
  for (size_t i = 0; i < var_names_.size(); ++i) {
    if (var_names_[i].substr(var_names_[i].length() - 4, 4) == "_pos" &&
      ...
      for (size_t j = i + 1; j < var_names_.size(); ++j) {
        if (var_names_[j] ==
            var_names_[i].substr(0, var_names_[i].length() - 4) + "_neg") {
          opposites_[i] = j;
          opposites_[j] = i;
          break;
        }
      ...
      for (size_t j = i + 1; j < var_names_.size(); ++j) {
        if (var_names_[j] ==
            var_names_[i].substr(0, var_names_[i].length() - 4) + "_pos") {
          opposites_[i] = j;
          opposites_[j] = i;
          break;
        }
...
bool Tableau::convertToCanonicalForm() {
  Tableau phase1(mat().size1(), mat().size2() + mat().size1(), var_names_,
                 &opposites_);
  phase1.mat()(0, 0) = 1;
  ...
  project(phase1.mat(), range(1, phase1.mat().size1()),
          range(mat().size1(), phase1.mat().size2())) =
      project(mat(), range(1, mat().size1()), range(0, mat().size2()));

  phase1.selectBasicVars();
  phase1.priceOut();
  if (!phase1.makeOptimal(true)) {
    throw std::runtime_error("Unable to convert LP tableau to canonical form, "
                             "likely due to unbounded feasible region.");
  }
  ...
  if (optimum == 0) {
        ...
        if (kvp.second < mat().size1()) {
          ...
          for (size_t j = mat().size1(); j < phase1.mat().size2() - 1; ++j) {
            if (phase1.mat()(kvp.first, j) != 0) {
              ...
              // we can pivot here without messing anything up without further
              // checks
    ...
    throw std::runtime_error(
        "Internal error: Optimized LP artificial variables below 0.");
  } else {
...
bool Tableau::makeOptimal(bool already_canonical) {
  ...
  // Select pivot col
  Rational max_obj_coeff =
      0; // If the max is <= 0, we're done, so feel free to start there
...
void Tableau::selectBasicVars() {
  // Makes basic_vars_ a map pointing from each row (other than 1st) to the
  // basic var column for it
  ...
  for (size_t j = 1; j < mat().size2() - 1; ++j) {
    ...
    // Otherwise, search each column; if it contains all 0s and one 1, it's
    // basic
    size_t row_with_1 = 0; // A 1 in the objective won't count, so we can also
                           // use it for columns where no 1 is ever found.
    bool is_basic = true;
    for (size_t i = 1; i < mat().size1(); ++i) {
      // Ok if objective is nonzero, but will need to price out later, so start
      // at 1
    ...
    if (is_basic) {
      ...
      if (!basic_vars_.count(row_with_1)) {
        ...
        mat().addRowMultToRow(0, row_with_1,
                              -mat()(0, j) / mat()(row_with_1, j));
      }
  ...
  throw std::runtime_error(
      "Tableau::selectBasicVars called on a Tableau " + mat().toString() +
      " with too few basic vars (found " + std::to_string(basic_vars_.size()) +
      ", need " + std::to_string(mat().size1() - 1) + ").");
}

void Tableau::priceOut() {
  for (const auto &kvp : basic_vars_) {
    mat().addRowMultToRow(0, kvp.first,
                          -mat()(0, kvp.second) / mat()(kvp.first, kvp.second));
  }
}
...
Rational Tableau::reportObjectiveValue() const {
  return mat()(0, mat().size2() - 1);
}
