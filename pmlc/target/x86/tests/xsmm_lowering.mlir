...
func @dot(%A: memref<4x8xf32>, %B: memref<8x6xf32>, %C: memref<4x6xf32>) -> () {
  ...
  affine.parallel (%i, %j, %k) = (0, 0, 0) to (8, 8, 8) step (2, 2, 2) {
    ...
    // CHECK-SAME: (%[[c8]], %[[c6]], %[[c6]], %[[c2]], %[[c2]], %[[c2]], %{{.*}}) : (i32, i32, i32, i32, i32, i32, memref<*xi64>) -> ()
    // CHECK: call @plaidml_rt_xsmm_exec_gemm_f32
    // CHECK-SAME: (%{{.*}}, %{{.*}}, %{{.*}}, %{{.*}}) : (memref<*xf32>, memref<*xf32>, memref<*xf32>, memref<*xi64>) -> ()
  }
  return
}
...
func @res2a_branch2a(%I: memref<1x56x56x64xf32>, %K: memref<1x1x64x64xf32>, %O: memref<1x56x56x64xf32>) -> () {
  ...
  affine.parallel (%x, %y) = (0, 0) to (56, 56) step (14, 1) {
    ...
    // CHECK: memref_cast
    // CHECK: call @plaidml_rt_xsmm_dispatch_gemm_f32
    // CHECK-SAME: (%[[c3584]], %[[c64]], %[[c3584]], %[[c14]], %[[c64]], %[[c64]], %{{.*}}) : (i32, i32, i32, i32, i32, i32, memref<*xi64>) -> ()
    // CHECK: call @plaidml_rt_xsmm_exec_gemm_f32
    // CHECK-SAME: (%{{.*}}, %{{.*}}, %{{.*}}, %{{.*}}) : (memref<*xf32>, memref<*xf32>, memref<*xf32>, memref<*xi64>) -> ()
  }
  return
