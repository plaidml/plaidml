...
func @dot(%A: memref<4x8xf32>, %B: memref<8x6xf32>, %C: memref<4x6xf32>) -> memref<4x6xf32> {
  %ret = affine.parallel (%i, %j, %k) = (0, 0, 0) to (8, 8, 8) step (2, 2, 2) reduce ("assign") -> (memref<4x6xf32>) {
    %0 = pxa.generic (%C[%i, %j]:#map) = @tpp_gemm(%A[%i, %k]:#map, %B[%k, %j]:#map) tile: [2, 2, 2]
...
func @res2a_branch2a(%I: memref<1x56x56x64xf32>, %K: memref<1x1x64x64xf32>, %O: memref<1x56x56x64xf32>) -> memref<1x56x56x64xf32> {
  %0 = affine.parallel (%i, %j) = (0, 0) to (56, 2) reduce ("assign") -> (memref<1x56x56x64xf32>) {
    %1 = pxa.generic (%O[0, 0, %i, %j * 32]: #map0) = @tpp_gemm(%I[0, 0, %i, 0]: #map0, %K[0, 0, 0, %j * 32]: #map1) tile: [56, 32, 64]
...
// CHECK:         xsmm.gemm.dispatch.f32 [56, 32, 64], [3584, 64, 3584]
// CHECK:         affine.for
// CHECK:           affine.for
// CHECK:             xsmm.gemm.invoke.f32
