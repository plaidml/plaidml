...
// Command lines:
// bazel-bin/pmlc/opt -convert-linalg-to-loops -x86-convert-pxa-to-affine -lower-affine  -canonicalize -convert-scf-to-std -x86-convert-std-to-llvm pmlc/target/x86/tests/conv_NHWC.mlir | bazel-bin/pmlc/jit -e baseline 

// bazel-bin/pmlc/opt -convert-linalg-to-loops -x86-affine-stencil-xsmm -x86-convert-pxa-to-affine -lower-affine  -canonicalize -convert-scf-to-std -x86-convert-std-to-llvm pmlc/target/x86/tests/conv_NHWC.mlir | bazel-bin/pmlc/jit -e baseline 

!I_memref = type memref<1x56x56x64xf32> // NHWC
!K_memref = type memref<1x1x64x64xf32>  // kh-kw-nofm-nifm
...
func @conv2(%I: !I_memref, %K: !K_memref, %O: !O_memref) {
  ...
  affine.parallel (%x, %y, %ci, %co) = (0, 0, 0, 0) to (56, 56, 64, 64) reduce ("assign") -> (memref<1x56x56x64xf32>) {
    ...
    %1 = pxa.load %K[0, 0, %ci, %co] : !K_memref
    %2 = mulf %0, %1 : f32
...
func @initK(%origK: !K_memref) {
  affine.parallel (%kh, %kw, %ci, %co) = (0, 0, 0, 0) to (1, 1, 64, 64) {
    ...
    affine.store %ar5, %origK[0, 0, %ci, %co] : !K_memref
  }
