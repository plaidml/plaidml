...
// CHECK-LABEL: func.func @no_gemm_mul_reduce_operation
//       CHECK:   affine.parallel
//   CHECK-NOT:     pxa.generic
func.func @no_gemm_mul_reduce_operation(%arg0: memref<100x100xf32>, %arg1: memref<100x100xf32>) -> memref<100x100xf32> {
...
// CHECK-LABEL: func.func @no_gemm_no_mul_before_reduce_operation
//       CHECK:   affine.parallel
//   CHECK-NOT:     pxa.generic
func.func @no_gemm_no_mul_before_reduce_operation(%arg0: memref<100x100xf32>, %arg1: memref<100x100xf32>) -> memref<100x100xf32> {
  %out = memref.alloc() : memref<100x100xf32>
  %ret = affine.parallel (%i, %j, %k) = (0, 0, 0) to (100, 100, 100) reduce ("assign") -> (memref<100x100xf32>) {
    %0 = pxa.load %arg1[%i, %k] : memref<100x100xf32>
    %1 = pxa.load %arg0[%k, %j] : memref<100x100xf32>
...
// CHECK-LABEL: func.func @no_gemm_mul_params_not_affine_loads
//       CHECK:   affine.parallel
//   CHECK-NOT:     pxa.generic
func.func @no_gemm_mul_params_not_affine_loads(%arg0: memref<100x100xf32>, %arg1: memref<100x100xf32>) -> memref<100x100xf32> {
  %out = memref.alloc() : memref<100x100xf32>
  %ret = affine.parallel (%i, %j, %k) = (0, 0, 0) to (100, 100, 100) reduce ("assign") -> (memref<100x100xf32>) {
    %0 = pxa.load %arg1[%i, %k] : memref<100x100xf32>
    %1 = pxa.load %arg0[%k, %j] : memref<100x100xf32>
...
// CHECK-LABEL: func.func @no_gemm_no_stride_one_1
//       CHECK:   affine.parallel
//   CHECK-NOT:     pxa.generic
func.func @no_gemm_no_stride_one_1(%arg0: memref<100x100xf32>, %arg1: memref<100x100xf32>) -> memref<100x100xf32> {
  %out = memref.alloc() : memref<100x100xf32>
  %ret = affine.parallel (%i, %j, %k) = (0, 0, 0) to (100, 100, 100) reduce ("assign") -> (memref<100x100xf32>) {
...
// CHECK-LABEL: func.func @no_gemm_no_stride_one_2
//       CHECK:   affine.parallel
//   CHECK-NOT:     pxa.generic
func.func @no_gemm_no_stride_one_2(%arg0: memref<100x100xf32>, %arg1: memref<100x100xf32>) -> memref<100x100xf32> {
  %out = memref.alloc() : memref<100x100xf32>
  %ret = affine.parallel (%i, %j, %k) = (0, 0, 0) to (100, 100, 100) reduce ("assign") -> (memref<100x100xf32>) {
    %0 = pxa.load %arg1[%k, %i] : memref<100x100xf32>
...
// CHECK-LABEL: func.func @gemm_operation_rewrite_f32
//       CHECK:   affine.parallel
//       CHECK:     pxa.generic
func.func @gemm_operation_rewrite_f32(%arg0: memref<100x100xf32>, %arg1: memref<100x100xf32>) -> memref<100x100xf32> {
  %out = memref.alloc() : memref<100x100xf32>
  %ret = affine.parallel (%i, %j, %k) = (0, 0, 0) to (100, 100, 100) reduce ("assign") -> (memref<100x100xf32>) {
    %0 = pxa.load %arg1[%i, %k] : memref<100x100xf32>
    %1 = pxa.load %arg0[%k, %j] : memref<100x100xf32>
...
// CHECK-LABEL: func.func @conv1
//       CHECK:   affine.parallel
//       CHECK:     pxa.generic
func.func @conv1(%arg0: memref<1x230x230x3xf32>, %arg1: memref<7x7x3x64xf32>, %arg2: memref<1x112x112x64xf32>) -> memref<1x112x112x64xf32> {
...
// CHECK-LABEL: func.func @res2a_branch2a
//       CHECK:   affine.parallel
//       CHECK:     pxa.generic
func.func @res2a_branch2a(%arg0: memref<1x56x56x64xf32>, %arg1: memref<1x1x64x64xf32>, %arg2: memref<1x56x56x64xf32>) -> memref<1x56x56x64xf32> {
...
// CHECK-LABEL: func.func @use_schedule
//       CHECK:   affine.parallel (%[[I0:.*]]) = (0) to (56)
//       CHECK:     pxa.generic (%{{.*}}[0, %[[I0]], 0, 0]: #{{.*}}) <addf>
//  CHECK-SAME:       @tpp_gemm(%{{.*}}[0, %[[I0]], 0, 0]: #{{.*}}, %{{.*}}[0, 0, 0, 0]: #{{.*}}) tile: [56, 64, 64]
//   CHECK-NOT:   {schedule = #{{.*}}}
func.func @use_schedule(%arg0: memref<1x56x56x64xf32>, %arg1: memref<1x1x64x64xf32>, %arg2: memref<1x56x56x64xf32>) -> memref<1x56x56x64xf32> {
...
// CHECK-LABEL: func.func @partial_schedule
...
//   CHECK-NOT:   {schedule = #{{.*}}}
func.func @partial_schedule(%arg0: memref<1x56x56x64xf32>, %arg1: memref<1x1x64x64xf32>, %arg2: memref<1x56x56x64xf32>) -> memref<1x56x56x64xf32> {
  %2 = affine.parallel (%n, %h, %w, %k, %r, %s, %c) = (0, 0, 0, 0, 0, 0, 0) to (1, 56, 56, 64, 1, 1, 64) reduce ("assign") -> (memref<1x56x56x64xf32>) {
    %6 = pxa.load %arg0[%n, %h + %r, %w + %s, %c] : memref<1x56x56x64xf32>
    %7 = pxa.load %arg1[%r, %s, %c, %k] : memref<1x1x64x64xf32>
    %8 = arith.mulf %6, %7 : f32
