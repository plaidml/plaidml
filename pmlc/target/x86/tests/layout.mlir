...
// func @main(%I0: tensor<1x56x56x64xf32>, %F0: tensor<1x1x64x64xf32> {stdx.const}, %B0: tensor<64xf32>) -> tensor<1x56x56x64xf32> {
//   %zero = tile.constant(0.0 : f64) : tensor<f32>
//   %F1 = tile.contract assign, none, %zero, %F0 {srcs = [#filter0], sink = #filter1}
//     : tensor<f32>, tensor<1x1x64x64xf32> -> tensor<4x4x1x1x16x16xf32>
//   %I1 = tile.contract assign, none, %zero, %I0 {srcs = [#input0], sink = #input1}
//     : tensor<f32>, tensor<1x56x56x64xf32> -> tensor<1x4x56x56x16xf32>
//   %0 = tile.contract add, mul, %zero, %I1, %F1 {
//     srcs = [#input, #filter],
//     sink = #output
//   } : tensor<f32>, tensor<1x4x56x56x16xf32>, tensor<4x4x1x1x16x16xf32> -> tensor<1x4x56x56x16xf32>
//   %1 = tile.contract assign, none, %zero, %0 { srcs = [#output0], sink = #output1}
//     : tensor<f32>, tensor<1x4x56x56x16xf32> -> tensor<1x56x56x64xf32>
...
func @main(%I0: tensor<1x56x56x64xf32>, %F0: tensor<1x1x64x64xf32> {stdx.const}, %B0: tensor<64xf32> {stdx.const}) -> tensor<1x56x56x64xf32> {
  ...
  // %4 = linalg.generic {indexing_maps = [#map3, #map5],
  ...
  // return %4 : tensor<1x56x56x64xf32>
  // %zero = tile.constant(0.0 : f64) : tensor<f32>
  // %0 = tile.contract add, mul, %zero, %I0, %F0 {sink = #output, srcs = [#input, #filter]} : tensor<f32>, tensor<1x56x56x64xf32>, tensor<1x1x64x64xf32> -> tensor<1x56x56x64xf32>
  // %1 = tile.add %0, %B0 : (tensor<1x56x56x64xf32>, tensor<64xf32>) -> tensor<1x56x56x64xf32>
  // %2 = tile.relu %1 : (tensor<1x56x56x64xf32>) -> tensor<1x56x56x64xf32>
  // return %2 : tensor<1x56x56x64xf32>

  // %2 = linalg.generic {indexing_maps = [], iterator_types = []}
  //   ins()
  //   outs() {
