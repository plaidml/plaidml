// RUN: pmlc-opt -split-input-file -x86-stage1 -x86-stage2="threads=8" %s
// TODO:  | FileCheck %s --check-prefix=STAGE2
// RUN: pmlc-opt -split-input-file -x86-stage1 -x86-stage2="threads=8" -x86-stage3 %s
// TODO:  | FileCheck %s --check-prefix=STAGE3
...
//     STAGE2: #[[MAP0:.*]] = affine_map<(d0, d1, d2) -> (0, d0, 0, d1)>
//     STAGE2: #[[MAP1:.*]] = affine_map<(d0, d1, d2, d3, d4) -> (0, d0 * 2 + d3, d4, d2)>
...
// STAGE2-DAG:       memref.alloc() : memref<1x16x1x64xf32>
//     STAGE2:       affine.parallel (%{{.*}}, %{{.*}}) = (0, 0) to (64, 16) reduce ("assign") -> (memref<1x112x112x64xf32>)
//     STAGE2:         pxa.reduce assign %{{.*}}, %{{.*}}[0, %{{.*}} + %{{.*}} * 16, %{{.*}} + %{{.*}} * 14, %{{.*}}] : memref<1x112x112x64xf32>
//     STAGE2:       pxa.generic (%{{.*}}[0, %{{.*}} * 16, %{{.*}} + %{{.*}} * 14, 0]: #[[MAP0]]) <addf> @tpp_gemm(%{{.*}}[0, %{{.*}} * 32, %{{.*}} * 2 + %{{.*}} * 28, 0]: #[[MAP1]], %{{.*}}[0, 0, 0, 0]: #[[MAP2]]) tile: [16, 64, 3, 1, 7, 7] : (memref<1x230x230x3xf32>, memref<7x7x3x64xf32>) -> memref<1x112x112x64xf32>
//     STAGE2:       affine.parallel (%{{.*}}, %{{.*}}) = (0, 0) to (64, 16) reduce ("assign") -> (memref<1x16x1x64xf32>)
//     STAGE2:         pxa.load %{{.*}}[0, %{{.*}} + %{{.*}} * 16, %{{.*}} + %{{.*}} * 14, %{{.*}}] : memref<1x112x112x64xf32>
//     STAGE2:         pxa.load %{{.*}}[%{{.*}}] : memref<64xf32>
//     STAGE2:         addf %{{.*}}, %{{.*}} : f32
//     STAGE2:         pxa.reduce assign %{{.*}}, %{{.*}}[0, %{{.*}}, 0, %{{.*}}] : memref<1x16x1x64xf32>
//     STAGE2:       memref.dealloc %{{.*}} : memref<1x112x112x64xf32>
//     STAGE2:       pxa.generic (%{{.*}}[0, %{{.*}} * 16, %{{.*}} * 14 + %{{.*}}, 0]: #[[MAP3]]) <assign> @tpp_relu(%{{.*}}[0, 0, 0, 0]: #[[MAP4]]) tile: [64, 16] : (memref<1x16x1x64xf32>) -> memref<1x112x112x64xf32>
