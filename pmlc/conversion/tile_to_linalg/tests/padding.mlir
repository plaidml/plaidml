...
// CHECK-SAME:     ins({{.*}} : tensor<12xf32>)
// CHECK-SAME:     outs({{.*}} : tensor<10xf32>)
// CHECK-SAME:     iterator_ranges = [10, 3]
//      CHECK:     addf
//      CHECK:     linalg.yield
...
//      CHECK: #[[map2:.*]] = affine_map<(d0, d1) -> (d0 + 2)>
//      CHECK: #[[map3:.*]] = affine_map<(d0, d1) -> (d0)>
//      CHECK: func.func @pad_contraction
...
// CHECK-SAME:     ins({{.*}}, {{.*}} : tensor<10xf32>, tensor<1xf32>)
// CHECK-SAME:     outs({{.*}} : tensor<12xf32>)
// CHECK-SAME:     iterator_ranges = [9, 1]
//      CHECK:     mulf
//      CHECK:     addf
//      CHECK:     linalg.yield
//      CHECK:   linalg.fill
//      CHECK:   linalg.generic
// CHECK-SAME:     indexing_maps = [#[[map0]], #[[map1]], #[[map3]]]
// CHECK-SAME:     iterator_types = ["parallel", "reduction"]
...
func.func @max_pool(%arg0: tensor<1x112x112x128xf32>, %arg1: tensor<3x3x128x256xf32> {stdx.const}) -> tensor<1x56x56x256xf32> {
...
//      CHECK: #[[map0:.*]] = affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d4 + d1 * 2, d5 + d2 * 2, d3)>
//      CHECK: #[[map1:.*]] = affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1 + 1, d2 + 1, d3)>
//      CHECK: #[[map2:.*]] = affine_map<(d0, d1, d2, d3, d4, d5, d6) -> (d0, d1 + d4, d2 + d5, d6)>
//      CHECK: #[[map3:.*]] = affine_map<(d0, d1, d2, d3, d4, d5, d6) -> (d4, d5, d6, d3)>
//      CHECK: #[[map4:.*]] = affine_map<(d0, d1, d2, d3, d4, d5, d6) -> (d0, d1, d2, d3)>
...
//      CHECK:   linalg.generic
// CHECK-SAME:     indexing_maps = [#[[map0]], #[[map1]]]
// CHECK-SAME:     iterator_types = ["parallel", "window", "window", "parallel", "reduction", "reduction"]
// CHECK-SAME:     ins(%{{.*}} : tensor<1x112x112x128xf32>)
// CHECK-SAME:     outs(%{{.*}} : tensor<1x58x58x128xf32>)
// CHECK-SAME:     iterator_ranges = [1, 56, 56, 128, 2, 2]
//      CHECK:     cmpf ogt
//      CHECK:     select
//      CHECK:     linalg.yield
