...
// CHECK-SAME: ins(%{{.*}}, %{{.*}} : tensor<10x20xf32>, tensor<20x30xf32>) outs(%{{.*}} : tensor<10x30xf32>) 
// CHECK:   mulf %{{.*}}, %{{.*}} : f32
// CHECK:   addf %{{.*}}, %{{.*}} : f32
...
// CHECK: linalg.fill
// CHECK: linalg.generic 
// CHECK-SAME: ins(%{{.*}}, %{{.*}} : tensor<10x30xf32>, tensor<30x40xf32>) outs(%{{.*}} : tensor<10x40xf32>)
// CHECK:   mulf %{{.*}}, %{{.*}} : f32
// CHECK:   addf %{{.*}}, %{{.*}} : f32
// CHECK:   linalg.yield %{{.*}} : f32
...
func @trace(
  ...
  %cst = tile.constant(0.0 : f64) : tensor<f32>
  %0 = tile.contract add, mul, %cst, %arg0, %arg1 {sink = #map0, srcs = [#map1, #map2], trace = "dot1"} :
    tensor<f32>, tensor<10x20xf32>, tensor<20x30xf32> -> tensor<10x30xf32>
  %1 = tile.contract add, mul, %cst, %0, %arg2 {sink = #map0, srcs = [#map1, #map2], trace = "dot2"} :
    tensor<f32>, tensor<10x30xf32>, tensor<30x40xf32> -> tensor<10x40xf32>
  return %1 : tensor<10x40xf32>
}
