...
TileToPXATypeConverter::TileToPXATypeConverter() {
  ...
  addConversion([](IntegerType type) {
    return pmlc::dialect::tile::toSignlessType(type);
  });
...
static void updateAffineMap(Operation *in,
...
buildBroadcastLoad(OpBuilder &builder, Location loc, Value operand,
                   unsigned outRank,
                   Optional<pmlc::dialect::tile::PaddingInfo> maybePadding) {
  ...
  auto loadOp =
      builder.create<pmlc::dialect::pxa::PxaLoadOp>(loc, operand, operandIdxs);
  if (maybePadding)
...
Value buildSimpleStore(
    OpBuilder &builder, Location loc, Value scalar, Value memRef,
    Optional<pmlc::dialect::tile::PaddingInfo> maybePadding) {
  ...
  auto storeOp = builder.create<pmlc::dialect::pxa::PxaReduceOp>(
...
BufferAllocator::BufferAllocator(OpBuilder &builder, Operation *op,
  ...
  auto maybePadding = pmlc::dialect::tile::getPaddingInfo(op);
  if (maybePadding) {
