...
// Compute the number of indices needed
int ExpandReshapePass::computeNumIdxs(ArrayRef<int64_t> srcShape, ArrayRef<int64_t> dstShape) {
...
void ExpandReshapePass::expandReshape(ReshapeOp reshapeOp) {
  ...
  while (pSrc >= 0) {
    if (srcDims[pSrc] > 1) {
      unsigned idxId = numIdxs - idxs.size() - 1;
      assert(idxId >= 0);
