...
class Stencil {
 private:
  // Number of instructions that need to be presented in the ParallelOp region to be
  // Considered a Gemm operation.
  // For now they are affine.load, affined.load, mul(f/i), reduce add, terminator.
...
mlir::Optional<GemmOperationMatch> Stencil::getGemmOperation() {
  ...
  if (body->getOperations().size() != kNumValidInstrInGemmRegion) {
    IVLOG(3, "the ParallelOp region didn't have the right number of instructions for a Gemm");
    return ret;
  }
  ...
  op.walk([&](AffineReduceOp reduceOp) {
    ...
    if (!lhs || !rhs) {
      IVLOG(3, "the lhs or rhs of the mul operation are not an affne.load operations");
      return;
    }
