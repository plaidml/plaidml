...
namespace pmlc::dialect::pxa {
  ...
  private:
    ...
    void transform(TensorAndIndexPermutation perm, ArrayRef<int64_t> tileSize) {
      ...
      auto brgemm =
        bodyBuilder.create<pxa::PxaBRGemmOp>(op.getLoc(), c.memref.getType(),  //
          c.memref, AffineMapAttr::get(c.accessMap), AffineMapAttr::get(c.tileMap), //
  ...
  struct StencilBRGEMMPass : public PassWrapper<StencilBRGEMMPass, FunctionPass> {
    StencilBRGEMMPass() { assert(false && "StencilBRGEMMPass must be configured"); }
