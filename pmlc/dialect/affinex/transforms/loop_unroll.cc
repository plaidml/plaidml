...
struct AffinexLoopUnroll : public AffinexLoopUnrollBase<AffinexLoopUnroll> {
  void runOnFunction() override {
    DenseMap<Operation *, uint64_t> instrCount;
    const uint64_t kMaxInstructions = 12;

    getFunction().walk([&](Operation *op) {
      auto count = instrCount[op];
      ...
      instrCount[op->getParentOp()] += count;

      if (isa<AffineForOp>(op) && count <= kMaxInstructions) {
        AffineForOp forOp = dyn_cast<AffineForOp>(op);
