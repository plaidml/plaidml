...
void *ocl_create_execenv(void *device) {
  IVLOG(1, "ocl_create_execenv device=" << device);
...
void ocl_destroy_kernel(void *invocation, void *kernel) {
  IVLOG(1, "ocl_destroy_execenv env=" << invocation << ", kernel=" << kernel);
...
void *ocl_schedule_compute(void *invocation, void *kernel,              //
  ...
  IVLOG(1, "ocl_schedule_compute env=" << invocation << ", kernel=" << kernel);
  IVLOG(1, "  grid=(" << gws0 << ", " << gws1 << ", " << gws2 << ")");
  IVLOG(1, "  block=(" << lws0 << ", " << lws1 << ", " << lws2 << ")");
  va_list args;
...
void ocl_dealloc(void *invocation, void *memory) {
  IVLOG(1, "ocl_dealloc env=" << invocation << ", buffer=" << memory);
...
void *ocl_schedule_read(void *host, void *dev, void *invocation, uint64_t count,
                        ...) {
  IVLOG(1, "ocl_schedule_read env=" << invocation << ", host=" << host
                                    << ", dev=" << dev);
...
void *ocl_schedule_write(void *host, void *dev, void *invocation,
                         uint64_t count, ...) {
  IVLOG(1, "ocl_schedule_write env=" << invocation << ", host=" << host
                                     << ", dev=" << dev);
  std::vector<OpenCLEvent *> dependencies;
  va_list args;
  va_start(args, count);
...
void *ocl_schedule_barrier(void *invocation, uint64_t count, ...) {
  IVLOG(1, "ocl_schedule_barrier env=" << invocation);
  std::vector<OpenCLEvent *> dependencies;
  va_list args;
  va_start(args, count);
  for (unsigned i = 0; i < count; ++i) {
