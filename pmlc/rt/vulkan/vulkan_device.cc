...
void VulkanDevice::setResourceStorageClassBindingMap(
...
void VulkanDevice::setResourceData(
...
void VulkanDevice::setResourceData(const ResourceData &resData) {
  if (!curr) {
    llvm::errs() << "setResourceData: curr is nullptr!";
    return;
  }
...
LogicalResult VulkanDevice::mapStorageClassToDescriptorType(
...
LogicalResult VulkanDevice::mapStorageClassToBufferUsageFlag(
...
LogicalResult VulkanDevice::countDeviceMemorySize() {
  if (!curr) {
...
LogicalResult VulkanDevice::checkResourceData() {
  if (!curr) {
    llvm::errs() << "checkResourceData: curr is nullptr!";
    return failure();
  }
  if (!curr->resourceData.size()) {
    llvm::errs() << "Vulkan device needs at least one resource";
    return failure();
  }
...
LogicalResult
VulkanDevice::createLaunchKernelAction(uint8_t *shader, uint32_t size,
...
LogicalResult VulkanDevice::createMemoryTransferAction(VkBuffer src,
                                                       VkBuffer dst,
                                                       size_t size) {
...
LogicalResult VulkanDevice::createMemoryTransferAction(uint64_t src_index,
...
LogicalResult VulkanDevice::setLaunchKernelAction() {
...
void VulkanDevice::addLaunchActionToSchedule() {
  schedule.push_back(curr);
  curr = nullptr;
}

LogicalResult VulkanDevice::submitCommandBuffers() {
...
LogicalResult
VulkanDevice::getBestComputeQueue(const VkPhysicalDevice &physicalDevice) {
...
LogicalResult VulkanDevice::createMemoryBuffers() {
  if (!curr) {
...
LogicalResult VulkanDevice::createShaderModule() {
  if (!curr) {
...
void VulkanDevice::initDescriptorSetLayoutBindingMap() {
  if (!curr) {
...
LogicalResult VulkanDevice::createDescriptorSetLayout() {
  if (!curr) {
...
LogicalResult VulkanDevice::createPipelineLayout() {
  if (!curr) {
...
LogicalResult VulkanDevice::createComputePipeline() {
  if (!curr) {
...
LogicalResult VulkanDevice::createDescriptorPool() {
  if (!curr) {
...
LogicalResult VulkanDevice::allocateDescriptorSets() {
  if (!curr) {
...
LogicalResult VulkanDevice::setWriteDescriptors() {
  if (!curr) {
...
LogicalResult VulkanDevice::submitCommandBuffersToQueue() {
...
LogicalResult VulkanDevice::updateHostMemoryBuffers() {
  for (const auto &action : schedule) {
    if (auto kernel = std::dynamic_pointer_cast<LaunchKernelAction>(action)) {
      // For each descriptor set.
...
LogicalResult VulkanDevice::createSchedule() {
