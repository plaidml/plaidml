...
parser.add_argument('--style-image',
                    type=str,
                    default='examples/style/portrait.jpg',
                    help='path to image to learn style from')
parser.add_argument('--style-mask',
                    type=str,
                    default='examples/style/portrait_mask.png',
                    help='path to semantic mask of style image')
parser.add_argument('--target-mask',
                    type=str,
                    default='examples/style/plaid_mask.png',
...
parser.add_argument('--target-image-prefix',
                    type=str,
                    default='ns_',
...
def kmeans(xs, k):
    ...
    try:
        ...
        _, labels = kmeans2(xs, k, missing='throw')
    return labels
...
def load_mask_labels():
    ...
    while True:
        try:
            ...
            print("Can't find {} clusters".format(num_labels))
            continue
    ...
    style_mask = np.stack([style_mask_label == r for r in xrange(num_labels)], axis=stack_axis)
    target_mask = np.stack([target_mask_label == r for r in xrange(num_labels)], axis=stack_axis)
