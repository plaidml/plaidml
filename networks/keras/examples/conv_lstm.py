        ...
        for j in range(n):
            ...
            for t in range(n_frames):
                ...
                if np.random.randint(0, 2):
                    ...
                    noisy_movies[i, t, x_shift - w - 1:x_shift + w + 1, y_shift - w - 1:y_shift +
                                 w + 1, 0] += noise_f * 0.1
...
# TODO: Only timing the training above, not this predict
for j in range(16):
    new_pos = seq.predict(track[np.newaxis, ::, ::, ::, ::])
    new = new_pos[::, -1, ::, ::, ::]
    track = np.concatenate((track, new), axis=0)

# TODO: Will be useless for correctness tests until beginning de-randomized
#output.contents = track
