    ...
    def get_batch(self, index, size, train):
        ...
        for i in range(0, size):
            ...
            if train and i > size - 4:
                ...
                if K.image_data_format() == 'channels_first':
                    X_data[i, 0, 0:self.img_w, :] = self.paint_func(self.X_text[index +
                                                                                i])[0, :, :].T
                else:
                    X_data[i, 0:self.img_w, :, 0] = self.paint_func(self.X_text[index +
                                                                                i])[0, :, :].T
    ...
    def next_train(self):
        while 1:
            ...
            if self.cur_train_index >= self.val_split:
                ...
                (self.X_text, self.Y_data,
                 self.Y_len) = shuffle_mats_or_lists([self.X_text, self.Y_data, self.Y_len],
                                                     self.val_split)
            yield ret
...
def train(run_name, start_epoch, stop_epoch, img_w):
    ...
    model.fit_generator(generator=img_gen.next_train(),
                        ...
                        callbacks=[viz_cb, img_gen, history, sw_manager],
                        initial_epoch=start_epoch)

    output.contents = np.array([history.loss, history.val_loss])
