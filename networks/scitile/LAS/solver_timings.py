...
labels = ['Numpy Linalg', 'Numpy Conjgrad','SciPy Dense','SciPy Sparse','PlaidML CG Dense','PlaidML CG Sparse']
...
for i in range(len(testnames)):
    ...
    testdat = np.load('test_data/'+testname+'.npz') # networks/scitile/LAS/
...
for i in range(len(testnames)):
    print(str(Ns[i])+','+str(times[i,:]).replace('[','').replace(']','').replace('\'','').replace('  ',' ').replace(' ',','))