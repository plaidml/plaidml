...
with plaidml.open_first_device(ctx) as dev:
    ...
    # Update R Function
    OR = tile.Operation("""function (A[I,J], LCC, C) -> (OR) {
                                TMP[i: I] = +(A[i,C]);
                                OR = TMP/LCC;
                            }
                            """, [('A', A), ('LCC', LCC), ('C', C)], [('OR', tile.Shape(dtype, np_R.shape))]).outputs['OR']
    ...
    err = 100*np.mean(np.abs((np_L[abs(true_chol)>1e-2]-true_chol[abs(true_chol)>1e-2])/true_chol[abs(true_chol)>1e-2]))
    print("Error: ",err)
