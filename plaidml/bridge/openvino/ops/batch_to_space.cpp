...
void registerBatchToSpace() {
  registerOp("BatchToSpace", [](const Context& ctx) {
    ...
    for (size_t i = 1; i < I.rank(); i++) {
      ...
      // Note that for constructing the batch index for I, we need to reverse the iteration order, as the largest
      // strides correspond to the earliest indexes for this
