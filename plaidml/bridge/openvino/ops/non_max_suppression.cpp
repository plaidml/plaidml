...
std::vector<Tensor> NMS(Tensor BOXES, Tensor SCORES, int32_t max_output_boxes_per_class, Tensor IOU_THRESHOLD,
  ...
  Tensor ZERO = cast(Tensor(0), box_input_type);
  Tensor ZERO_THRES = cast(Tensor(0), thres_type);
  Tensor ONE_THRES = cast(Tensor(1), thres_type);
  Tensor NEG1 = cast(Tensor(-1), box_input_type);
  Tensor NEG1_OUTPUT = cast(Tensor(-1), box_output_type);
  ...
  Tensor WEIGHT = select(SOFT_NMS_SIGMA != 0.0f, -0.5 / SOFT_NMS_SIGMA, ZERO_THRES);
  ...
  for (int i = 0; i < num_batches; i++) {
    ...
    for (int j = 0; j < num_classes; j++) {
      ...
      Tensor NODE = cast(edsl::Constant(buffer_node, llvm::formatv("X{0}-{1}", i, j)), box_output_type);

      // Select box
      for (int k = 0; k < num_boxes_per_class; k++) {
        // Select the box with largest score
        Tensor CANDIDATE_INDEX = reshape(op::argmax(NEW_SCORES, Value(2)), {TensorDim(1)});             // 1*ui32
        Tensor SCORE = reshape(gather(NEW_SCORES, CANDIDATE_INDEX).axis(2), {one_td, one_td, one_td});  // 1*1*1
        Tensor CURRENT_NODE = select(SCORE > 0.0f, NODE, cast(INVALID_NODE, box_output_type));

        // Update count of selected box
        Tensor VALID = select(SCORE != 0.0f, ONE_THRES, ZERO_THRES);
        VALID_OUTPUTS = VALID_OUTPUTS + VALID;

        // Add selected box to scores
        scores.push_back(cast(CURRENT_NODE, thres_type));
        SCORE = select(SCORE > 0.0f, SCORE, NEG1);
        scores.push_back(cast(SCORE, thres_type));
        ...
        // Add select box to boxes
        Tensor BOX_INDEX = select(SCORE > 0.0f, cast(CANDIDATE_INDEX, box_output_type), NEG1_OUTPUT);
        boxes.push_back(CURRENT_NODE);
        boxes.push_back(reshape(BOX_INDEX, {one_td, one_td, one_td}));
  ...
  VALID_OUTPUTS = cast(VALID_OUTPUTS, box_output_type);
  ...
  if (sort_result_descending) {
    ...
    SCORES_SLICE = reshape(SCORES_SLICE, {num_results_td});
    Tensor INDEXES = argsort(SCORES_SLICE, 0, SortDirection::DESC);
