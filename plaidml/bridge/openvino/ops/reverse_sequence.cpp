    ...
    for (int64_t i = 0; i < shapes[batch_axis]; i++) {
      ...
      for (int64_t j = 0; j < shapes.size(); j++) {
        if (j == batch_axis) {
          ...
          if (length[i] < 1 || length[i] >= shapes[seq_axis]) {
            THROW_IE_EXCEPTION << " unreasonable sequence length; ";
          }
      ...
      edsl::Tensor reverse = op::unsqueeze(reverse_piece, {batch_axis});
      edsl::Tensor constant = op::unsqueeze(constant_piece, {batch_axis});
