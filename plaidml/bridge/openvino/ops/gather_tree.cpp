...
Tensor GatherTree(Tensor STEP_IDS, Tensor PARENT_IDX, Tensor MAX_SEQ_LEN, Tensor END_TOKEN) {
  ...
  Tensor INDEX_BB = op::slice(INDEX_BEAM).add_dim(0, 1).add_dim(0, batch_size).add_dim(0, beam_width);
  ...
  for (int i = max_time - 1; i >= 0; i--) {
    ...
    Tensor PARENT_IDX_S = op::squeeze(
        op::slice(PARENT_IDX_COR).add_dim(i, i + 1).add_dim(0, batch_size).add_dim(0, beam_width).add_dim(0, 2), {0});
    Tensor NEW_PARENT =
  ...
  Tensor PARENTS_ND_T = edsl::select(PARENTS_ND_FILTER < 0, PARENTS_ND,
                                     op::unsqueeze(edsl::cast(INDEX_O, PARENTS_ND.dtype()), {0}));  // 2*2*2*2  1*2*2*2
  Tensor PARENTS_ND_NEW = op::concatenate({op::unsqueeze(INDEX_TIME, {-1}), PARENTS_ND_T}, -1);     // 2*2*2*1 2*2*2*2
