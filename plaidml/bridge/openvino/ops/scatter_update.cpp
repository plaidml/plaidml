...
void registerScatterUpdate() {
  registerOp("ScatterUpdate", [](const Context& ctx) {
    auto* layer = ngraph::as_type<ngraph::opset4::ScatterUpdate>(ctx.layer);
    ...
    auto O = edsl::scatter(data, indices, updates).axis(axis).mode(edsl::ScatterMode::UPDATE_SLICE);
