...
void registerGroupConvolutionBackpropData() {
  registerOp("GroupConvolutionBackpropData", [](const Context& ctx) {
    auto* layer = ngraph::as_type<ngraph::opset1::GroupConvolutionBackpropData>(ctx.layer);
    ...
    if (ctx.operands.size() == 3) {
      auto result_shape = layer->get_convolution_output_shape();
