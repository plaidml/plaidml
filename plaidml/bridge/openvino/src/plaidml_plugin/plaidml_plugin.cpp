...
Engine::LoadExeNetworkImpl(const InferenceEngine::ICore * /* core */, InferenceEngine::ICNNNetwork &network,
...
INFERENCE_PLUGIN_API(StatusCode) CreatePluginEngine(IInferencePlugin*& plugin, ResponseDesc *resp) noexcept {
    try {
        plugin = make_ie_compatible_plugin(
                {{1, 6},
                 CI_BUILD_NUMBER,
                 "PlaidMLPlugin"}, std::make_shared<Engine>());
        return OK;
    }
...
InferenceEngine::Parameter PlaidMLPlugin::Engine::GetMetric(const std::string& name,
                                                            const std::map<std::string,
                                                                           InferenceEngine::Parameter>&) const {
