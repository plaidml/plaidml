...
TEST(Op, BinaryCrossentropy) {
  ...
  Program program("binary_crossentropy", {op::binary_crossentropy(I, O, 0.0f)});
  IVLOG(1, program);
  EXPECT_THAT(program, Eq(R"#(
...
module {
  func @binary_crossentropy(%arg0: tensor<7x7x3x64x!eltwise.f32> {tile.name = "O"}, %arg1: tensor<7x7x3x64x!eltwise.f32> {tile.name = "I"}) -> tensor<7x7x3x64x!eltwise.f32> {
    %cst = "eltwise.sconst"() {value = 1.000000e+00 : f32} : () -> !f32
    %cst_0 = "eltwise.sconst"() {value = 0.000000e+00 : f32} : () -> !f32
    %c1 = "eltwise.sconst"() {value = 1 : i64} : () -> !i64
    ...
    %4 = "eltwise.sub"(%c1, %3) : (!i64, tensor<7x7x3x64x!eltwise.f32>) -> tensor<7x7x3x64x!eltwise.f32>
...
TEST(Op, Elu) {
  auto I = Placeholder(DType::FLOAT32, {7, 7, 3, 64}, "I");
  Program program("elu", {op::elu(I, 0.1f)});
  IVLOG(1, program);
  EXPECT_THAT(program, Eq(R"#(
...
module {
  func @elu(%arg0: tensor<7x7x3x64x!eltwise.f32> {tile.name = "I"}) -> tensor<7x7x3x64x!eltwise.f32> {
    %cst = "eltwise.sconst"() {value = 1.000000e-01 : f32} : () -> !f32
    %c0 = "eltwise.sconst"() {value = 0 : i64} : () -> !i64
...
module {
  func @hard_sigmoid(%arg0: tensor<10x20x!eltwise.f32> {tile.name = "A"}) -> tensor<10x20x!eltwise.f32> {
    %cst = "eltwise.sconst"() {value = 5.000000e-01 : f32} : () -> !f32
    %cst_0 = "eltwise.sconst"() {value = 5.000000e-02 : f32} : () -> !f32
    %cst_1 = "eltwise.sconst"() {value = 1.000000e+00 : f32} : () -> !f32
    %cst_2 = "eltwise.sconst"() {value = 1.000000e+01 : f32} : () -> !f32
    %cst_3 = "eltwise.sconst"() {value = 0.000000e+00 : f32} : () -> !f32
    %cst_4 = "eltwise.sconst"() {value = -1.000000e+01 : f32} : () -> !f32
...
TEST(Op, Relu) {
  ...
  Program program("relu", {op::relu(I).alpha(A).max_value(M).threshold(0.05f)});
  EXPECT_THAT(program, Eq(R"#(
...
module {
  func @relu(%arg0: tensor<10x20x!eltwise.f32> {tile.name = "M"}, %arg1: tensor<10x20x!eltwise.f32> {tile.name = "I"}, %arg2: tensor<10x20x!eltwise.f32> {tile.name = "A"}) -> tensor<10x20x!eltwise.f32> {
    %cst = "eltwise.sconst"() {value = 5.000000e-02 : f32} : () -> !f32
...
TEST(Op, ReluNoAlpha) {
  auto I = Placeholder(DType::FLOAT32, {10, 20}, "I");
  auto M = Placeholder(DType::FLOAT32, {10, 20}, "M");
  Program program("relu", {op::relu(I).max_value(M).threshold(0.05f)});
  EXPECT_THAT(program, Eq(R"#(
...
TEST(Op, ReluNoMaxValue) {
  auto I = Placeholder(DType::FLOAT32, {10, 20}, "I");
  auto A = Placeholder(DType::FLOAT32, {10, 20}, "A");
  Program program("relu", {op::relu(I).alpha(A).threshold(0.05f)});
  EXPECT_THAT(program, Eq(R"#(
...
TEST(Op, ReluOnlyThreshold) {
  auto I = Placeholder(DType::FLOAT32, {10, 20}, "I");
  Program program("relu", {op::relu(I).threshold(0.05f)});
  EXPECT_THAT(program, Eq(R"#(
