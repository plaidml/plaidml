...
TEST_F(OpTest, Abs) {
  auto I = Input(DType::FLOAT32, {1, 224, 224, 3}, "I");
...
TEST_F(OpTest, BinaryCrossentropy) {
  auto I = Input(DType::FLOAT32, {7, 7, 3, 64}, "I");
  auto O = Input(DType::FLOAT32, {7, 7, 3, 64}, "O");
  auto program = makeProgram("binary_crossentropy", {op::binary_crossentropy(I, O, 0.0)});
  runProgram(program);
}

TEST_F(OpTest, BroadcastNoOp) {
  auto A = Input(DType::FLOAT32, {3});
...
TEST_F(OpTest, BroadcastNoOpLarge) {
  auto A = Input(DType::FLOAT32, {3, 4});
  std::vector<int> rshape = {3, 4};
...
TEST_F(OpTest, BroadcastNumpy) {
  auto A = Input(DType::FLOAT32, {1, 3, 3});
...
TEST_F(OpTest, BroadcastNumpy2) {
  auto A = Input(DType::FLOAT32, {3, 1});
  std::vector<int> rshape = {3, 4};
  std::vector<int> bdims = {0, 1};
  auto C = op::broadcast(A, rshape, bdims);
...
TEST_F(OpTest, Broadcast) {
  auto I = Input(DType::FLOAT32, {1, 224, 224}, "I");
...
TEST_F(OpTest, Clip) {
  auto I = Input(DType::FLOAT32, {7, 7, 3, 64}, "I");
  auto raw_min = Input(DType::FLOAT32, {7, 7, 3, 64}, "raw_min");
  auto raw_max = Input(DType::FLOAT32, {7, 7, 3, 64}, "raw_max");
  auto program = makeProgram("clip", {op::clip(I, raw_min, raw_max)});
  runProgram(program);
}

TEST_F(OpTest, Concatenate) {
  auto A = Input(DType::FLOAT32, {7, 7, 3, 64}, "A");
  auto B = Input(DType::FLOAT32, {7, 7, 3, 64}, "B");
...
TEST_P(ConvolutionTest, Basic) {
  ...
  auto I = Input(params.dType, params.I_dims);
  auto K = Input(params.dType, params.K_dims);
...
TEST_F(OpTest, Dot) {
  auto I = Input(DType::FLOAT32, {7, 7, 3, 64}, "I");
  auto K = Input(DType::FLOAT32, {7, 7, 3, 64}, "K");
...
TEST_F(OpTest, ExplicitPadding) {
  auto I = Input(DType::FLOAT32, {2, 3}, "A");
...
TEST_F(OpTest, HardSigmoid) {
  auto A = Input(DType::FLOAT32, {10, 20}, "A");
...
TEST_F(OpTest, Maximum) {
  auto A = Input(DType::FLOAT32, {10, 20}, "A");
  auto B = Input(DType::FLOAT32, {10, 20}, "B");
...
TEST_F(OpTest, Relu) {
  auto I = Input(DType::FLOAT32, {10, 20}, "I");
  auto A = Input(DType::FLOAT32, {10, 20}, "A");
  auto M = Input(DType::FLOAT32, {10, 20}, "M");
...
TEST_F(OpTest, ReorgYoloDecrease) {
  ...
  auto I = Input(DType::INT64, {N, C, H, W});
...
TEST_F(OpTest, Repeat) {
  auto A = Input(DType::FLOAT32, {32, 1, 4, 1}, "A");
...
TEST_F(OpTest, Sigmoid) {
  auto A = Input(DType::FLOAT32, {10}, "A");
...
TEST_F(OpTest, SpatialPadding) {
  auto A = Input(DType::FLOAT32, {64, 4, 32, 32}, "A");
...
TEST_F(OpTest, Squeeze) {
  auto A = Input(DType::UINT64, {3, 1, 4, 1}, "A");
...
TEST_F(OpTest, Unsqueeze) {
  auto A = Input(DType::UINT64, {3, 4}, "A");
