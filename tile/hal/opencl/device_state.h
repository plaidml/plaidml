...
class DeviceState {
 public:
  ...
  void Initialize(const context::Context& ctx, const hal::proto::HardwareSettings& settings);
  ...
  const Queue& cl_queue() const { return *cl_queue_; }
 ...
 private:
  ...
  std::unique_ptr<const Queue> cl_queue_;
