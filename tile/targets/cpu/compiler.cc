...
#define CPU_THREAD_TAG_NAME "cpu_thread"
#define XSMM_TAG_NAME "xsmm"
#define XSMM_RT_CALLER_NAME "XSMMRTCaller"
#define PARALLEL_COLLECTION_CREATOR_NAME "CreateParallelCollection"
#define PARALLEL_EXECUTE_NAME "ParallelExecute"
...
Compiler::Compiler(llvm::LLVMContext* context, const Config& config)
    : context_(*context), builder_{context_}, config_{config}, arenaSize_(0), stackLocation(nullptr) {
...
ProgramModule Compiler::CompileProgram(const stripe::Block& program) {
  ...
  llvm::Function* main;
  llvm::Value* parallelTaskListPtr;
  std::tie(main, parallelTaskListPtr) = CompileBlock(program);
...
Compiler::Compiler(llvm::LLVMContext* context, llvm::Module* module, const Config& config)
    : context_(*context), builder_{context_}, module_(module), config_{config}, arenaSize_(0), stackLocation(nullptr) {
...
void Compiler::GenerateInvoker(const stripe::Block& program, llvm::Function* main) {
  ...
  args.push_back(builder_.CreateIntToPtr(IndexConst(0), builder_.getIntPtrTy(module_->getDataLayout())));
...
llvm::Function* Compiler::CompileXSMMBlock(const stripe::Block& block, const XSMMDispatch xsmmDispatch,
  ...
  llvm::Function* function = llvm::Function::Create(func_type, linkage, name, module_);

  // Areate a basic block; configure the builder to start there
  auto bb = llvm::BasicBlock::Create(context_, "entry", function);
  builder_.SetInsertPoint(bb);

  llvm::Value* parallelListPtr = getPassedParameters(block, function);
  ...
  std::vector<llvm::Type*> param_types{
      ...
      parallelListPtr->getType(),  // Pointer to the collection with work items for parallel execution.
      IndexType(),
  };
  ...
  auto xmmCallFunc = module_->getOrInsertFunction(XSMM_RT_CALLER_NAME, xsmmCallHelperType).getCallee();
  std::vector<llvm::Value*> args2 = {
      func,
      buffers_[xsmmCallData.in1->into()].base,
      buffers_[xsmmCallData.in0->into()].base,
      buffers_[xsmmCallData.out0->into()].base,
      parallelListPtr,
      IndexConst((int)ParallelCallType::XSMM),
  };
...
llvm::Value* Compiler::getPassedParameters(const stripe::Block& block, llvm::Function* function) {
  llvm::Value* parallelListPtr = nullptr;
  ...
  for (auto ai = function->arg_begin(); ai != function->arg_end(); ++ai) {
    ...
    if (idx < block.refs.size()) {
    ...
    } else if (idx < block.refs.size() + block.idxs.size()) {
      idx -= block.refs.size();
      ...
      idx -= block.refs.size() + block.idxs.size();
      if (idx != 0) {
        throw std::runtime_error("Wrong index for the parallel location parameter.");
      }
      parallelListPtr = &(*ai);
    }
  }

  return parallelListPtr;
}

std::tuple<llvm::Function*, llvm::Value*> Compiler::CompileBlock(const stripe::Block& block) {
  if (block.has_tag(XSMM_TAG_NAME)) {
    ...
    if (xsmmDispatch != XSMMDispatch::NONE && data) {
      // The XSMM block should be a leaf block with no any nested blocks.
      for (const auto& stmt : block.stmts) {
        if (stripe::Block::Downcast(stmt)) {
          throw std::runtime_error("XSMM block should not have nested blocks.");
        }
      }
      return std::make_tuple(CompileXSMMBlock(block, xsmmDispatch, xsmmCallData), nullptr);
    }
  }
  ...
  // Check to see if there are direct nested blocks with #cpu_thread.
  // If so, create a collection on the stack to store the parallel
  // items that need to be run.
  bool hasParallelSubBlocks = false;
  bool isParallelXSMMBlock = false;
#if PLAID_USE_TBB_PARALLELISM
  // For XSMM case make sure we have only one block child that has XSMM tag on it.
  for (const auto& stmt : block.stmts) {
    if (auto inner = stripe::Block::Downcast(stmt)) {
      if (block.has_tag(CPU_THREAD_TAG_NAME)) {
        hasParallelSubBlocks = true;
        // For XSMM case make sure we have only one block child that has XSMM tag on it.

        if (inner->has_tag(XSMM_TAG_NAME)) {
          if (block.stmts.size() != 1) {
            throw std::runtime_error("In case of parallel XSMM blocks the parent block should have a single child.");
          }

          isParallelXSMMBlock = true;
        }
      }
    }
  }

  if (hasParallelSubBlocks) {
    // Allocate a stack slot for the collection to keep the parallelItems for this block.
    stackLocation = builder_.CreateAlloca(builder_.getIntPtrTy(module_->getDataLayout()), nullptr, block.name);
    // Set nullptr in the slot.
    builder_.CreateStore(builder_.CreateIntToPtr(IndexConst(0), builder_.getIntPtrTy(module_->getDataLayout())),
                         stackLocation);

    if (isParallelXSMMBlock) {
      // Create a call to initialized the parallel items collection.
      std::vector<llvm::Type*> param_types{
          stackLocation->getType(),
          builder_.getInt64Ty(),
      };

      llvm::FunctionType* callType = llvm::FunctionType::get(builder_.getVoidTy(), param_types, false);
      llvm::FunctionType* parallelCallHelperType = llvm::FunctionType::get(builder_.getVoidTy(), param_types, false);
      auto parallelCreateFunc =
          module_->getOrInsertFunction(PARALLEL_COLLECTION_CREATOR_NAME, parallelCallHelperType).getCallee();

      std::vector<llvm::Value*> args1 = {
          stackLocation,
          IndexConst(ParallelCallType::XSMM),
      };

      builder_.CreateCall(callType, parallelCreateFunc, args1);
    }
  }
  ...
  std::tuple<llvm::Function*, llvm::Value*> ret = std::make_tuple(function, nullptr);
  ...
  if (hasParallelSubBlocks) {
    if (isParallelXSMMBlock) {
      ...
      llvm::FunctionType* callType = llvm::FunctionType::get(builder_.getVoidTy(), param_types, false);
      llvm::FunctionType* parallelCallHelperType = llvm::FunctionType::get(builder_.getVoidTy(), param_types, false);
      auto parallelExecuteFunc =
          module_->getOrInsertFunction(PARALLEL_EXECUTE_NAME, parallelCallHelperType).getCallee();
      std::vector<llvm::Value*> args1 = {
          stackLocation,
          IndexConst(ParallelCallType::XSMM),
      };

      builder_.CreateCall(callType, parallelExecuteFunc, args1);
    }
  }
#endif  // PLAID_USE_TBB_PARALLELISM
...
void Compiler::Visit(const stripe::Block& block) {
  ...
  llvm::Value* parallelListPtr;
  std::tie(function, parallelListPtr) = nested.CompileBlock(block);
  ...
  if (stackLocation != nullptr) {
    args.push_back(builder_.CreateLoad(stackLocation));
  } else {
    args.push_back(builder_.CreateIntToPtr(IndexConst(0), builder_.getIntPtrTy(module_->getDataLayout())));
  }
...
llvm::FunctionType* Compiler::BlockType(const stripe::Block& block) {
  ...
  param_types.push_back(builder_.getIntPtrTy(module_->getDataLayout()));
...
void Compiler::EmitRunTimeLogEntry(const std::string& str, const std::string& extra, llvm::Value* value) {
  ...
  if (value) {
    ...
    log_args.push_back(llvm::ConstantInt::get(builder_.getInt64Ty(), 0));
  }
