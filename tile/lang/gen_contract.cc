...
KernelInfo GenContract(const string& kname, const DirectSettings& settings, const FlatContraction& op,
  ...
  for (size_t i = 0; i < 3; i++) {
    ki.lwork[i] = (i == 0 ? settings.threads : 1);
  ...
  if (!settings.use_global) {
    ...
    for (size_t i = 1; i < op.access.size(); i++) {
      if (bindings[i] && bindings[i]->tag == Binding::TENSOR) {
        ...
        iblock->push_back(pins[i - 1].generate(sname + "_shared", sname, settings.threads,
                                               op.access[i].global_index_limit, op.access[i].offset));
        loaded_input = true;
      }
    }
  ...
  uint64_t out_threads = settings.threads / rthreads;
  ...
  auto looped_inner = ci_i.generate(settings.threads, out_threads);
  ...
  auto slow_looped_inner = dup_ci_i.generate(settings.threads, out_threads);
  ...
  // Maybe merge block
  uint64_t comp_threads = settings.threads / rthreads;
  if (out_threads < comp_threads) {
    auto mblock = _Block({});
    sem::Type ltype = {sem::Type::VALUE, op.access[0].type, op.access[0].vector, settings.threads, sem::Type::LOCAL};
  ...
  if (op.agg_op == AggregationOp::MAX) {
    wblock->append(o = _Cond(o == agg_min, agg_zero, o));
  }
  ...
  wblock->append(_If(check, assign));

  kblock->append(ci2.generate(settings.threads, 1, false, false));

  // Wrap entire function
  auto func = std::make_shared<sem::Function>();
  func->name = kname;
  func->ret = {sem::Type::TVOID};
  sem::Type out_type = {sem::Type::POINTER_MUT, op.access[0].type, op.access[0].vector, 0, sem::Type::GLOBAL};
  func->params.emplace_back(out_type, std::string("out"));
  for (size_t i = 1; i < op.access.size(); i++) {
    if (bindings[i] && bindings[i]->tag == Binding::TENSOR) {
