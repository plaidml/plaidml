...
class Matrix : public boost::numeric::ublas::matrix<Rational> {
 public:
  ...
  Matrix(size_type size1, size_type size2) : boost::numeric::ublas::matrix<Rational>(size1, size2) {}
  ...
  // These one-param ctors are not explicit b/c boost does not make them explicit;
  // so presumably implicit conversion is an intended use
  Matrix(const boost::numeric::ublas::matrix<Rational>& m)  // NOLINT(runtime/explicit)
      : boost::numeric::ublas::matrix<Rational>(m) {}
  template <class AE>
  Matrix(const boost::numeric::ublas::matrix_expression<AE>& ae)  // NOLINT(runtime/explicit)
