...
void LightCstrReduction(const AliasMap& alias_map, Block* block, const proto::LightConstraintReductionPass& options) {
  ...
  for (const auto& constraint : block->constraints) {
    ...
    for (auto kvp_it = constraint_map.begin(); kvp_it != constraint_map.end();) {
      ...
      // Then: coeff*idx >= -rest
      //       -coeff*idx <= rest
      //       idx <= rest / -coeff  (rounding division upwards -- N.B. -coeff is positive)
      //       idx <= RoundUp(rest, -coeff)
      // And since the maximum value (rest) might take on is lim_value-1,
      //       idx <= RoundUp(lim_value-1, -coeff)
      // So:
      ...
      //       idx >= -rest / coeff  (rounding division upwards -- N.B. coeff is positive)
      //       idx >= RoundUp(-rest, coeff)
      // And since the minimum value (rest) might take on is min_value,
      //       idx >= RoundUp(-min_value, coeff)
      //
      // So, define shift=min(range(idx), RoundUp(-min_value, coeff)).
      //
      // If shift>0, we can rewrite all uses of the index, eliding the [0..shift) range that is being skipped
      // over by this constraint:
      //       idx' = idx + shift
      ...
      if (0 < coeff) {
        std::int64_t shift = std::min(static_cast<int64_t>(idx->range), RoundUp(-min_value, coeff));
        if (0 < shift) {
