...
std::unique_ptr<Executable> Compiler::CompileProgram(const stripe::Block& program) {
  // Compile each block in this program into a function within a single LLVM module.
  module_ = new llvm::Module("stripe", context_);
  llvm::Function* main = CompileBlock(program);
  // Generate a stub function we can invoke from the outside, passing the buffers in
  ...
  // optimization passes; among many other things, this will streamline our loops
  // to eliminate most of the branching and inline most of the block function calls.
  ...
  // Wrap the finished module and the buffer names into an executable we can return.
